...
const ConvertStringToCode = async () => {
  ...
  client.query("select distinct marca from carros").then((res) => {
    ...
    fs.writeFileSync("Mapeamento/MapMarca.csv", "", (err) =>
      console.log(err.message)
    );
    res.rows.forEach((linha, index) => {
      fs.writeFile(
        "Mapeamento/MapMarca.csv",
        `${index},${linha.marca}\n`,
        { flag: "a" },
        (err) => {
  ...
  client.query("select distinct tipoveiculo from carros").then((res) => {
    ...
    fs.writeFileSync("Mapeamento/MapTipoVeiculo.csv", "", (err) =>
      console.log(err.message)
    );
    res.rows.forEach((linha, index) => {
      fs.writeFile(
        "Mapeamento/MapTipoVeiculo.csv",
        `${index},${linha.tipoveiculo}\n`,
        { flag: "a" },
        (err) => {
          if (err) console.log(err.message);
          bar.tick();
  ...
  client.query("select distinct combustivel from carros").then((res) => {
    ...
    fs.writeFileSync("Mapeamento/MapCombustivel.csv", "", (err) =>
      console.log(err.message)
    );
    res.rows.forEach((linha, index) => {
      fs.writeFile(
        "Mapeamento/MapCombustivel.csv",
        `${index},${linha.combustivel}\n`,
        { flag: "a" },
        (err) => {
          if (err) console.log(err.message);
          bar.tick();
  ...
  client.query("select distinct cambio from carros").then((res) => {
    ...
    fs.writeFileSync("Mapeamento/MapCambio.csv", "", (err) =>
      console.log(err.message)
    );
    res.rows.forEach((linha, index) => {
      fs.writeFile(
        "Mapeamento/MapCambio.csv",
        `${index},${linha.cambio}\n`,
        { flag: "a" },
        (err) => {
          if (err) console.log(err.message);
  ...
  client.query("select distinct direcao from carros").then((res) => {
    ...
    fs.writeFileSync("Mapeamento/MapDirecao.csv", "", (err) =>
      console.log(err.message)
    );
    res.rows.forEach((linha, index) => {
      fs.writeFile(
        "Mapeamento/MapDirecao.csv",
        `${index},${linha.direcao}\n`,
        { flag: "a" },
        (err) => {
          if (err) console.log(err.message);
  ...
  client.query("select distinct cor from carros").then((res) => {
    ...
    fs.writeFileSync("Mapeamento/MapCor.csv", "", (err) =>
      console.log(err.message)
    );
    res.rows.forEach((linha, index) => {
      fs.writeFile(
        "Mapeamento/MapCor.csv",
        `${index},${linha.cor}\n`,
        { flag: "a" },
        (err) => {
          if (err) console.log(err.message);
  ...
  client.query("select distinct modelo from carros").then((res) => {
    ...
    fs.writeFileSync("Mapeamento/MapModelo.csv", "", (err) =>
      console.log(err.message)
    );
    res.rows.forEach((linha, index) => {
      fs.writeFile(
        "Mapeamento/MapModelo.csv",
        `${index},${linha.modelo}\n`,
        { flag: "a" },
        (err) => {
          if (err) console.log(err.message);
