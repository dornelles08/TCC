...
def coletaLinksPage(page):
    ...
    result = requests.get(
        "https://sp.olx.com.br/sao-paulo-e-regiao/autos-e-pecas/carros-vans-e-utilitarios?o="+str(page), headers=header)
    html = result.content
...
def main():
    ...
    for l in links:
        try:
            dados = coletaDados(l)
            requests.post('http://localhost:2005/saveOnMongo', json=dados)
        except:
